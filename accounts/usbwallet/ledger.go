...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Pdexchain app in browser mode", w.failure
	}
	if w.offline() {
		return "Pdexchain app offline", w.failure
	}
	return fmt.Sprintf("Pdexchain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
